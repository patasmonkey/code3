...
# 3 "SDSoCyou9_float.c" 2
# 1 "/usr/local/Cellar/gcc/7.1.0/lib/gcc/7/gcc/x86_64-apple-darwin16.5.0/7.1.0/include-fixed/math.h" 1 3 4
# 48 "/usr/local/Cellar/gcc/7.1.0/lib/gcc/7/gcc/x86_64-apple-darwin16.5.0/7.1.0/include-fixed/math.h" 3 4
...
# 7 "SDSoCyou9_float.c" 2


# 8 "SDSoCyou9_float.c"
FILE *fin,*fout,*fp;
# 20 "SDSoCyou9_float.c"
float table_kx[180*360];
float table_ky[180*360];
float table_kz[180];
...
float Kai[(180 +1)*(360 +1)];


float Ur_r[201*201*201];
float Ur_i[201*201*201];



int Max_Phi;
int Max_Theta,Max_Theta_2;

int Energy_min,Energy_step,No_Incident;
...
int main(int ardc,const char * argv[]){
  ...
  gettimeofday(&tv1,
# 47 "SDSoCyou9_float.c" 3 4
                   ((void *)0)
# 47 "SDSoCyou9_float.c"
                       );
  for(sa=0;sa<5;sa++){
    ...
    int i,j,e;
    int dim1_ch,dim2_ch,dim3_ch,Phi_ch,Theta_ch;
    int Pict_size;
    float Pict_step,dim1_min,dim2_min,dim3_min,Phi_rotation;
    float Theta,Phi,Energy_fl,Energy[21];
    float kx,ky,kz,kr,x,y,z,k[21];
    float kr_2,kk;
    float Energy_wt[21];
    float TT2;
    float STT2;
    ...
    float Sum_Theta_0,Sum_Theta_1,Sum_Phi_0,Sum_Phi_1,krkr[2];
    int a,b,c;
    ...
    for(i=0;i<No_Incident;i++){
      Energy_fl=Energy_min+Energy_step*i;
      Energy[i]=Energy_fl/1000;
      k[i]=0.5067731235l*Energy[i];
      Energy_wt[i]=pow(Energy[i]/Energy[0],2);
    }
    ...
    for(Phi_ch=0;Phi_ch<360;Phi_ch++){
      Phi=1.0l*Phi_ch*PI/180.0l+Phi_rotation*PI/180.0l;
      cosphi[Phi_ch]=cos(Phi);
      sinphi[Phi_ch]=sin(Phi);
    }

    for(Theta_ch=0;Theta_ch<180;Theta_ch++){
      Theta=1.0l*Theta_ch*PI/180.0l;
      table_kz[Theta_ch]=cos(Theta);
      sinth[Theta_ch]=sin(Theta);
      for(Phi_ch=0;Phi_ch<360;Phi_ch++){
 table_kx[Theta_ch*360 +Phi_ch]=cosphi[Phi_ch]*sinth[Theta_ch];
 table_ky[Theta_ch*360 +Phi_ch]=sinphi[Phi_ch]*sinth[Theta_ch];
      }
    }


    for(dim3_ch=0;dim3_ch<201;dim3_ch++){
      for(dim1_ch=0;dim1_ch<201;dim1_ch++){
 for(dim2_ch=0;dim2_ch<201;dim2_ch++){
   Ur_r[dim3_ch*(201*201)+dim1_ch*201 +dim2_ch]=Ur_i[dim3_ch*(201*201)+dim1_ch*201 +dim2_ch]=0.00l;
 }
      }
    }
 ...
 char filename_r2[200];
 int Energy_extention;
 char filename_r1[100]={"ex_zr"};
 Energy_extention=Energy_min+Energy_step*i;
 
# 138 "SDSoCyou9_float.c" 3 4
__builtin___sprintf_chk (
# 138 "SDSoCyou9_float.c"
filename_r2
# 138 "SDSoCyou9_float.c" 3 4
...
, 2 > 1 ? 1 : 0), 
# 138 "SDSoCyou9_float.c"
"%s%d.txt", filename_r1, Energy_extention
# 138 "SDSoCyou9_float.c" 3 4
)
# 138 "SDSoCyou9_float.c"
                                                               ;
 if ((fin=fopen(filename_r2, "r"))==
# 139 "SDSoCyou9_float.c" 3 4
                                   ((void *)0)
# 139 "SDSoCyou9_float.c"
                                       ){
   printf("File %s is not found!\n", filename_r2);
   return 2;
 }
 for(Phi_ch=0;Phi_ch<Max_Phi;++Phi_ch){
   for(Theta_ch=0;Theta_ch<=Max_Theta;++Theta_ch){
     j=fscanf(fin, "%f", &Kai[Theta_ch*(Max_Phi)+Phi_ch]);
   }
 }
 ...
 for(dim3_ch=0;dim3_ch<201;dim3_ch++){
   ...
   for(dim1_ch=0;dim1_ch<201;dim1_ch++){
     ...
     for(dim2_ch=0;dim2_ch<201;dim2_ch++){
       Sum_Theta_0=0.0;
       Sum_Theta_1=0.0;
  for(Theta_ch=1;Theta_ch<Max_Theta;Theta_ch++){
    ...
    for(Phi_ch=0;Phi_ch<Max_Phi;Phi_ch++){
      ...
      x=dim1_min+Pict_step*dim1_ch;
      y=dim2_min+Pict_step*dim2_ch;
      z=dim3_min+Pict_step*dim3_ch;
      r_norm=sqrtf(x*x+y*y+z*z);
      kk=k[i];
      kr_2=table_kx[Theta_ch*(Max_Phi)+Phi_ch]*x
        +table_ky[Theta_ch*(Max_Phi)+Phi_ch]*y
        +table_kz[Theta_ch]*z
        +r_norm;
      Sum_Phi_0+=Kai[Theta_ch*(Max_Phi)+Phi_ch]*cosf(kk*kr_2);
      Sum_Phi_1+=Kai[Theta_ch*(Max_Phi)+Phi_ch]*sinf(kk*kr_2);
    }
    Sum_Theta_0+=Sum_Phi_0*sinth[Theta_ch];
    Sum_Theta_1+=Sum_Phi_1*sinth[Theta_ch];
  }
  Ur_r[dim3_ch*(201*201)+dim1_ch*(201)+dim2_ch]+=Sum_Theta_0*Energy_wt[i];
  Ur_i[dim3_ch*(201*201)+dim1_ch*(201)+dim2_ch]+=Sum_Theta_1*Energy_wt[i];
     }
   }
 }
      ...
      if ((fp = fopen("data4", "w")) == 
# 193 "SDSoCyou9_float.c" 3 4
                                       ((void *)0)
# 193 "SDSoCyou9_float.c"
                                           ) {
 fprintf(
# 194 "SDSoCyou9_float.c" 3 4
        __stderrp
# 194 "SDSoCyou9_float.c"
              , "ファイルのオープンに失敗しました．\n");
 return 
# 195 "SDSoCyou9_float.c" 3 4
       1
# 195 "SDSoCyou9_float.c"
                   ;
      }
 ...
 for(dim1_ch=0;dim1_ch<201;dim1_ch++){
   for(dim2_ch=0;dim2_ch<201;dim2_ch++){
     TT2=sqrt(Ur_r[dim3_ch*(201*201)+dim1_ch*(201)+dim2_ch]*Ur_r[dim3_ch*((201)*(201))+dim1_ch*(201)+dim2_ch]
       +Ur_i[dim3_ch*((201)*(201))+dim1_ch*(201)+dim2_ch]*Ur_i[dim3_ch*((201)*(201))+dim1_ch*(201)+dim2_ch]
       *(-Ur_r[dim3_ch*((201)*(201))+dim1_ch*(201)+dim2_ch]));

     if(TT2>0) STT2=sqrt(TT2);
     else STT2=0;
     fprintf(fp,"%15.10f ",STT2);
   }
   TT2=sqrt(Ur_r[dim3_ch*((201)*(201))+dim1_ch*(201)+dim2_ch]*Ur_r[dim3_ch*((201)*(201))+dim1_ch*(201)+dim2_ch]
     +Ur_i[dim3_ch*((201)*(201))+dim1_ch*(201)+dim2_ch]*Ur_i[dim3_ch*((201)*(201))+dim1_ch*(201)+dim2_ch]
     *(-Ur_r[dim3_ch*((201)*(201))+dim1_ch*(201)+dim2_ch]));
   ...
   fprintf(fp,"%15.10f\n",STT2);
      ...
      fclose(fp);
      gettimeofday(&tv2,
# 219 "SDSoCyou9_float.c" 3 4
                       ((void *)0)
# 219 "SDSoCyou9_float.c"
                           );
      if ((fp = fopen("time1", "a")) == 
# 220 "SDSoCyou9_float.c" 3 4
                                       ((void *)0)
# 220 "SDSoCyou9_float.c"
                                           ) {
 fprintf(
# 221 "SDSoCyou9_float.c" 3 4
        __stderrp
# 221 "SDSoCyou9_float.c"
              , "ファイルのオープンに失敗しました．\n");
 return 
# 222 "SDSoCyou9_float.c" 3 4
       1
# 222 "SDSoCyou9_float.c"
                   ;
      }
      fprintf(fp,"acc4_f %d %ld %06lu\n",sa,tv2.tv_sec-tv1.tv_sec,tv2.tv_usec-tv1.tv_usec);
      fclose(fp);
